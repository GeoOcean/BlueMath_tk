...
def add_forcing_edges_to_computational_domain(poly_clip, vert, tria):
    ...
    - Edges with one endpoint inside: clip at the boundary.
    - Edges with both inside: split at hole boundaries if they cross a hole.
    - Edges with both outside: if the segment crosses the polygon, clip to the
      interior portion(s) and add those segments as forcing edges.
    """
    ...
    edges_both_outside = edges_new[count_inside == 0]

    new_points = []
    tol = 1e-10

    def point_to_index(pt, n1, n2, vert, new_points, tol):
        """Retourne l'index (dans vert_update) du point pt, ou ajoute un nouveau point."""
        p1, p2 = vert[n1], vert[n2]
        if np.linalg.norm(np.asarray(pt) - p1) <= tol:
            return n1
        if np.linalg.norm(np.asarray(pt) - p2) <= tol:
            return n2
        pt = np.asarray(pt, dtype=float)
        for k, q in enumerate(new_points):
            if np.linalg.norm(pt - np.asarray(q)) <= tol:
                return len(vert) + k
        new_points.append(pt)
        return len(vert) + len(new_points) - 1

    # ---- Arêtes avec un seul sommet inside : clip à la frontière
    ...
    # ---- Arêtes avec les deux sommets inside : découper aux trous si le segment traverse un trou
    edges_both_inside_result = []
    for (n1, n2) in edges_both_inside:
        p1, p2 = vert[n1], vert[n2]
        line = LineString([p1, p2])
        geom = line.intersection(poly_clip)
        ...
        if geom.geom_type == "LineString":
            ...
            if len(coords) >= 2 and np.linalg.norm(coords[0] - coords[-1]) > tol:
                edges_both_inside_result.append((n1, n2))
            continue
        if geom.geom_type == "MultiLineString":
            for seg in geom.geoms:
                ...
                start_pt = coords[0]
                end_pt = coords[-1]
                idx_start = point_to_index(start_pt, n1, n2, vert, new_points, tol)
                idx_end = point_to_index(end_pt, n1, n2, vert, new_points, tol)
                if idx_start != idx_end:
                    edges_both_inside_result.append((idx_start, idx_end))
            continue
        edges_both_inside_result.append((n1, n2))

    # ---- Arêtes avec les deux sommets outside : inclure si le segment traverse le polygone
    edges_both_outside_result = []
    for (n1, n2) in edges_both_outside:
        p1, p2 = vert[n1], vert[n2]
        line = LineString([p1, p2])
        geom = line.intersection(poly_clip)
        ...
        if geom.geom_type == "LineString":
            coords = np.array(geom.coords)
            if len(coords) >= 2 and np.linalg.norm(coords[0] - coords[-1]) > tol:
                idx_start = point_to_index(coords[0], n1, n2, vert, new_points, tol)
                idx_end = point_to_index(coords[-1], n1, n2, vert, new_points, tol)
                if idx_start != idx_end:
                    edges_both_outside_result.append((idx_start, idx_end))
            continue
        if geom.geom_type == "MultiLineString":
            for seg in geom.geoms:
                coords = np.array(seg.coords)
    ...
    # Sommets mis à jour et arêtes finales
    if len(new_points) > 0:
    ...
    parts = []
    if len(edges_both_inside_result) > 0:
        parts.append(np.array(edges_both_inside_result))
    parts.append(edges_one_inside_new)
    if len(edges_both_outside_result) > 0:
        parts.append(np.array(edges_both_outside_result))

    edges_used = np.vstack(parts) if parts else np.empty((0, 2), dtype=int)

    if edges_used.shape[0] == 0:
        return vert_update, np.empty((0, 2), dtype=int)
...
def plot_additive_input_wind_partition(
...
def plot_additive_vs_dynamic_windsetup_swath(
...
def plot_additive_vs_dynamic_windsetup(
    ds_WL_GS_WindSetUp: xr.Dataset,
    ds_WL_dynamic_WindSetUp: xr.Dataset,
    ds_gfd_metadata: xr.Dataset,
    time: datetime,
...
def plot_additive_TG_validation_timeseries(
    ds_WL_GS_WindSetUp: xr.Dataset,
...
def plot_additive_setup_with_raster(
    simulation_dataset,
    path_grd_calc,
    figsize=(7, 7),
) -> None:
    ...
    plot_additive_setup(simulation_dataset, figsize=(7, 7), ax=ax, fig=fig)
...
def plot_additive_validation_timeseries(
    ds_WL_GS_WindSetUp: xr.Dataset,
    ds_WL_GS_IB: xr.Dataset,
    ds_WL_dynamic_WindSetUp: xr.Dataset,
    ds_GFD_info: xr.Dataset,
...
def build_additive_infos_dataset(
    Nodes_calc,
    Elmts_calc,
    Nodes_forz,
    Elmts_forz,
...
def wind_partition_triangular(ds_GFD_info, xds_vortex):
    """
