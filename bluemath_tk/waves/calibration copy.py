...
class CalVal(BlueMathModel):
    """
    Calibrates wave reanalysis data with buoy and satellite data.
    Preferably calibrates first with satellites, then validates with buoys if available.

    Attributes
    ----------
    hindcast : pd.DataFrame
        Original hindcast data. Must contain columns:
    ...
    def __init__(
        ...
        self.possible_to_correct: np.ndarray = np.where(hindcast["Hs"] > 0.01)[0]
