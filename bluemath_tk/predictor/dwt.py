...
class DWT:
    ...
    def generate_dataset(self):
        ...
        for ivar, var in enumerate(self.predictor):
            ...
            if not var in ds.data_vars.keys():
                raise DWTError(
                    "Variable {0} not found in dataset {1}. \
            ...
            if not correct_dims:
                print(ds.dims)
                raise DWTError(
                    "Incorrect dimensions found in dataset. Expected dimensions: {0}'".format(
                        expected_dims
                    )
                )
            ...
            ds["longitude"] = (
                ds.longitude + 360
            ) % 360  # Make sure longitudes are 0-360
            ...
            if area[1] < area[0]:  # Area goes over 0
                ...
                ds = ds.isel(
                    longitude=np.where(
                        (ds.longitude.values > area[0] - 5)
                        | (ds.longitude.values < area[1] + 5)
                    )[0]
                )
            else:
                ds = ds.sel(
            ...
            if self.resample_factor > 1:
                DS = DS.coarsen(
                    longitude=resample_factor, latitude=resample_factor, boundary="pad"
                ).mean()
        ...
        if estela:
            ...
            ESTELA = ESTELA.assign(
                {
                    "estela_mask": (
                        ("latitude", "longitude"),
                        np.where(ESTELA.F.values > 0, 1, np.nan),
                    )
                }
            )

            # select ESTELA at site grid
            ESTELA_site = ESTELA.sortby(ESTELA.latitude, ascending=False).interp(
        ...
        if area[1] < area[0]:  # Area goes over 0
            ...
            DS = DS.isel(
                longitude=np.where(
                    (DS.longitude.values > area[0]) | (DS.longitude.values < area[1])
                )[0]
            )
    ...
    def dynamic_estela_predictor(self):
        ...
        for i_lat in range(len(xdset.latitude)):
            for i_lon in range(len(xdset.longitude)):
                ...
                if not np.isnan(ed):
                    ...
                    i_times = np.arange(first_day, len(xdset.time)) - int(ed)  # np.int

                    # select data from displaced time array positions
                    xdselec = xdset.isel(time=i_times, latitude=i_lat, longitude=i_lon)

                    # get estela predictor values
                    var_comp[:, i_lat, i_lon] = xdselec[var_name].values
                    var_grd_comp[:, i_lat, i_lon] = xdselec[
        ...
        return xr.Dataset(
            {
                "{0}".format(var_name): (("time", "latitude", "longitude"), var_comp),
                "{0}_grad".format(var_name): (
    ...
    def sort_bmus(self):
        ...
        order = np.argsort(
            self.dwt_centroids[var_order].mean(dim=("longitude", "latitude")).values
        )
        print(order)
    ...
    def plot_map_mean(self):
        ...
        for iv, var in enumerate(vars):
            ...
            vmin, vmax = (
                config_variables[var]["limits"][0],
                config_variables[var]["limits"][1],
            )
            ...
            if not estela:
                ax.set_extent(
                    [
                        ds.longitude.values.min() - 10,
                        ds.longitude.values.max() + 10,
                        ds.latitude.values.min() - 10,
                        ds.latitude.values.max() + 10,
                    ],
            ...
            plt.colorbar(contour, ax=ax, orientation="horizontal", shrink=0.7)
            ax.set_title(var)
    ...
    def plot_dwts(self, anomaly=False):
        ...
        for iv, var in enumerate(vars):
            fig, axs = plt.subplots(
                nrows,
                ncols,
                figsize=[20, 18 / (len(ds.longitude.values) / len(ds.latitude.values))],
                subplot_kw={
                    "projection": ccrs.PlateCarree(central_longitude=central_longitude)
                },
            )
            ...
            plt.subplots_adjust(
                left=0.02, right=0.98, top=0.92, bottom=0.01, wspace=0.05, hspace=0.05
            )
            ...
            if anomaly:
                ...
                lim = np.nanmax(
                    [
                        np.abs(np.nanpercentile(total, 1)),
                        np.abs(np.nanpercentile(total, 99)),
                    ]
                )
                vmin, vmax = -lim, lim
                cmap = "RdBu_r"
            ...
            for d in range(n_dwts):
                ...
                color_css = mcolors.to_hex(
                    np.array([np.append(np_colors_int[d], 0.5)]), keep_alpha=True
                )
            ...
            cbar = plt.colorbar(
                contour, ax=axs_list, orientation="vertical", shrink=0.6
            )
            cbar.set_label(config_variables[var]["label"], fontsize=16)

            fig.savefig(
                op.join(p_site, "plot_dwts_{}_anomaly_{}.png".format(var, anomaly)),
                bbox_inches="tight",
            )
            with open(
                op.join(p_site, "plot_dwts_{}_anomaly_{}.pkl".format(var, anomaly)),
                "wb",
            ) as f:
                pickle.dump(fig, f)
