...
class SchismHyCoFfEE(SchismModelWrapper):
    ...
    # -----------------------------
    # The default parameters for the wrapper.
    # These parameters are used to generate the input files for the model.
    ...
    def generate_time_series(
        ...
        # Duración teniendo en cuenta los desfases entre picos de caudal y storm surge/set-up
        # Puede ser que tau(ss/set-up-discahrge) sea negativo pero Tp sea tan pequeño que las 24 horas del surge empiecen antes que el hidrograma de caudal
        ...
        if (tau_ss_d<=0 and time0>=0 and tp!=0):   # el hidrograma marca el inicio del tiempo
            ...
            if (tau_ss_d<=0 and time0<0 and tp!=0):
                ...
                if tau_ss_d>0:
                    ...
                    if tp==0:  # el tiempo de concentración también, con lo que se supone que el caudal es constante e igual al base
        ...
        if (timef>np.max(hidro_hours) and tau_ss_d<0 and time0>0): #añadido nuevo el time0>0
            ...
            if (hidro_hours[-1]<48 and tau_ss_d<0  and time0>0):  #añadido nuevo el time0>0
        ...
        at_inicial = np.sin(np.pi/2 + np.pi*tau/6 + 2*np.pi*t_inicial/(12*3600))* cm/2
    ...
    def rndays_calculation(
        ...
        # Calculate the total seconds for the simulation
        total_seconds = (
            t_rndays * 24 * 3600
        )
    ...
    def generate_wind_th(
        ...
        #############################################################
        u_wind = -np.ones(len(times)) * wind_magnitude * np.cos(np.radians(wind_direction_SCHISM))
        v_wind = -np.ones(len(times)) * wind_magnitude * np.sin(np.radians(wind_direction_SCHISM))


        # u_wind = -[wind_magnitude * np.cos(np.radians(wind_direction))] * len(times)  # Calculate u_wind
        # v_wind = -[wind_magnitude * np.sin(np.radians(wind_direction))] * len(times)  # Calculate v_wind
    ...
    def generate_at_th(
        ...
        # OJO AQUI CON EL AT_SERIES, VER EL FORMATO EN EL QUE ENTRA!!!!
    ...
    def build_case(
        ...
        t_series, at_series, ss_series, AT_SS_series, qq_series = self.generate_time_series(
