...
def Normalize(data, ix_scalar, ix_directional, minis=[], maxis=[]):
    ...
    # directional data (max distance is pi or 180)
...
def nearest_indexes_weighted(data_q, data, ix_scalar, ix_directional, ix_weights):
    ...
    data_q_norm, _, _ = Normalize(
        data_q, ix_scalar, ix_directional, minis=minis, maxis=maxis
    )
    ...
    for c, dq in enumerate(data_q_norm):
        ...
        D = Normalized_Distance_weighted(
            data_norm, ddq, ix_scalar, ix_directional, ix_weights
        )
